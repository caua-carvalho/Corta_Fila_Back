<?php
require_once '../db.php'; // Tem que garantir que esse arquivo conecta certinho no banco

// Função de alto nível para autenticar barbeiro
// Recebe phone e password, retorna true se autenticado, false se não
function autenticarBarbeiro(string $phone, string $password): bool {
    ...
    // Busca o barbeiro pelo phone
    $usuario = buscarUsuarioPorPhone($phone);
    if (!$usuario) {
    ...
    echo json_encode(['mensagem' => 'Login realizado com sucesso', 'user' => $usuario]);
    return true;
}
...
function buscarUsuarioPorPhone(string $phone): ?array {
    global $pdo;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    autenticarBarbeiro($phone, $password);
} else {
